...
def algorithm_heuristic(participants: np.ndarray[int],
    ...
    order_diverse = [k for k, v in fields.items() if
                     v == FieldMode.Diversify_1 or v == FieldMode.Diversify_2 or v == FieldMode.Diversify_3]
    ...
    try:
        ...
        raise Exception("Error: Random seed incorrect!",
    ...
    n_results = allocate(tables, peopledata_vals_used, order_cluster_dict, order_diverse_dict, m_data, nallocations,
                         cluster_tables, pareto_prob, n_swap_loops, progress_bar, previous_meetings, no_cluster_agents,
                         val_cluster, manuals, random, fields)
    ...
    for index, label in enumerate(cat_labels):
        ...
        if table_dist[index] > ideal_dist[index]:
            for a, b in zip(table_discrepancies, cat_labels):
                ...
                elif diversity_val == FieldMode.Diversify_2 and a < -0.125:
                    actions_for_label.append(b)
