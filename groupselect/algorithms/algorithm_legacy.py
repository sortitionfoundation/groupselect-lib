...
def algorithm_legacy(
    ...
    """Allocate participants into groups using the legacy algorithm."""
    # Check argument: seed
    ...
    try:
        ...
        raise Exception(
            "Error when creating random number generator.",
            "This may be due to an incorrect seed. Please check your input.",
        )
    ...
    for field_id in fields:
        field_vals, field_val_counts = np.unique(
            participants.T[field_id], return_counts=True
        )
    ...
    allocation_attempts: list[AllocationEnsemble] = [
        AllocationEnsemble() for _ in range(n_attempts)
    ]
    ...
    for a_id in range(len(groups)):
        for sample, choice in zip(
            allocation_samples, random.choice(n_attempts, n_attempts)
        ):
...
def _allocate_legacy_once(
    ...
    allocation = Allocation(ParticipantGroup() for _ in range(n_gr))
    ...
    # Split fields into clustering and diversification fields. Order
    # participant IDs by clustering and diversification field values.
...
def _allocate_person(
    ...
    groups_list = [
        g_id for g_id, group in enumerate(allocation) if len(group) < n_ppgr
    ]
    ...
    for field_id in fields_cluster:
        ...
        field_value_counts = {
            g_id: _count_categories(
                allocation[g_id], field_id, field_val, participants
            )
            for g_id in groups_list
        }

        # Create a temporary list of potential groups to focus on. To
        # begin with, this list contains all groups that already contain
        # at least one participant of the same field value.
        groups_list_tmp = [
            g_id for g_id in groups_list if field_value_counts[g_id] > 0
        ]

        # Then check if the temporary list of groups is enough to
        # accommodate all participants with this specific field value.
        # If not, then keep adding one more group, which is determined
        # from all groups from groups_list
        spaces_required = _number_of_people_filtered(
            participants, superior_field_filters | {field_id: field_val}
        )
        groups_options = None
        while (
            sum((n_ppgr - len(allocation[g_id])) for g_id in groups_list_tmp)
            < spaces_required
        ):
    ...
    for field_id in fields_diversify:
        ...
        field_val_counts_min = min(
            field_value_counts.items(), key=lambda x: x[1]
        )[1]
        groups_list = [
            g_id
            for g_id in groups_list
    ...
    groups_list = [
        g_id for g_id in groups_list if len(allocation[g_id]) == group_size_min
    ]
...
def _count_categories(
    ...
    return sum(
        1 for p_id in group if participants[p_id][field_id] == field_val
    )


def _number_of_people_filtered(
    participants: np.ndarray[int], fields: dict[int, int]
):
    return (
        (participants.T[list(fields.keys())] == list(fields.values()))
        .all(axis=0)
        .sum()
    )
