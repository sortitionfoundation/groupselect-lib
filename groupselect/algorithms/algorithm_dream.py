...
def algorithm_dream(
    ...
    """Allocate participants into groups using the DREAM algorithm."""
    if not (isinstance(pareto_prob, float) and 0.0 <= pareto_prob <= 0.5):
        raise Exception(
            f"Pareto probability must be float in range [0.0, 0.5] but "
            f"found: {pareto_prob}"
        )
...
# --- Shared allocation machinery ---------------------------------------
#
# The functions below implement DREAM's core loop: build an initial random
# seating, then run rounds of Pareto swaps that trade off demographic
# balance against unique-meeting count. HERMES (algorithm_hermes.py) reuses
# all of it unchanged, overriding only `pareto_swaps` (injected into
# `run_round` below) to weigh each diversity field's swap-acceptance
# probability individually instead of using one fixed probability/threshold
# for every field. `evaluate_actions`, `evaluate_demographics`, and
# `select_key` accept optional `threshold`/`pareto_prob` overrides for the
# same reason, defaulting to DREAM's original fixed values.


def evaluate_meetings(table, previous_meetings):
    """Sum each agent's meeting count with the other agents at its table."""
    total_meetings = {}
...
def evaluate_swap(
    ...
    """Return the meeting-count change from swapping two agents' tables."""
...
def generate_combinations(demogs, info):
    """Count, per category combination, how many values differ from info."""
...
def calculate_ideal_balance(cats_diverse, m_data, people):
    """Return each category's target share of the whole participant pool."""
    ideal_balance = {}
...
def evaluate_actions(
    ideal_dist, table_dist, cat_labels, table_size, threshold: float = 0.0
):
    """For each over-represented label, list which labels to swap in."""
...
def evaluate_demographics(
    ...
    cats_diverse,
    m_data,
    threshold_func: Callable[[int], float] = lambda demog: 0.0,
):
    """Return a table's demographic imbalance and possible swap actions."""
    ...
    for demog in cats_diverse:
        ...
        table_actions[demog] = evaluate_actions(
            ...
            cats_diverse[demog],
            len(table),
            threshold=threshold_func(demog),
        )
...
def select_key(pareto, meet, pareto_prob, random):
    """Pick a swap candidate, favouring balance or unique meetings."""
    # `pareto_prob` is normally a fixed float (DREAM). HERMES instead
    # passes a callable that resolves the probability from the specific
    # candidate key chosen below, so it can weigh each diversity field
    # individually.
    if callable(pareto_prob):
        pareto_prob = pareto_prob(list(pareto.keys())[0])
...
def pareto_swaps(
    ...
    """Select the best swap for a round using a single fixed threshold."""
...
def run_round(
    ...
    pareto_prob,
    random,
    pareto_swaps=pareto_swaps,
) -> Allocation:
    """Build one allocation from a template, then run rounds of swaps."""
    ...
    if len(cats_cluster) == 1:
        ...
        if len(cluster_individuals) > total_clustering_spaces:
            raise ValueError(
                "Too many manual allocations to clustering tables: "
                "please reduce manual allocations."
            )
...
def allocate(
    ...
    """Build the requested number of allocation rounds and return them."""
